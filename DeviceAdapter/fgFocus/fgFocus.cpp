...
const char *g_CameraDeviceName = "fgFocus Light Sensor";
...
{
	RegisterDevice(g_CameraDeviceName, MM::CameraDevice, "fgFocus Light Sensor");
}
...
fgFocus::fgFocus() : CCameraBase<fgFocus>(),
					 initialized_(false)
{
	// call the base class method to set-up default error codes/messages
	InitializeDefaultErrorMessages();

	CPropertyAction *pAct = new CPropertyAction(this, &fgFocus::OnPort);
 ...
 */
void fgFocus::GetName(char *name) const
{
...
bool fgFocus::ReadExactBytes(const std::string &port, uint8_t *buffer, size_t expectedBytes, unsigned long timeoutMs)
{
	unsigned long totalRead = 0;
	unsigned long startTime = GetClockTicksUs() / 1000; // Convert �s to ms
...
int fgFocus::SnapImage()
{
	...
	for (int chunk = 0; chunk < 4; ++chunk)
	{
		...
		{
			// --- Read Lower Bytes ---
			if (!ReadExactBytes(port_, chunkLower.data(), 37, 200))
			{ // 200 ms timeout
			...
			// --- Read Upper Bytes ---
			if (!ReadExactBytes(port_, chunkUpper.data(), 37, 200))
			{ // 200 ms timeout
				LogMessage("Timeout waiting for chunk " + std::to_string(chunk) + " lower bytes.");
				return DEVICE_ERR;
			}
 ...
 */
const unsigned char *fgFocus::GetImageBuffer()
{
 ...
 * Required by the MM::Camera API.
 */
unsigned fgFocus::GetImageWidth() const
{
	return img_.Width();
}
 ...
 * Required by the MM::Camera API.
 */
unsigned fgFocus::GetImageHeight() const
{
	return img_.Height();
}
 ...
 * Required by the MM::Camera API.
 */
unsigned fgFocus::GetImageBytesPerPixel() const
{
	return img_.Depth();
}
 ...
 * Required by the MM::Camera API.
 */
unsigned fgFocus::GetBitDepth() const
{
	return bitDepth_;
}
 ...
 * Required by the MM::Camera API.
 */
long fgFocus::GetImageBufferSize() const
{
 ...
 * Required by the MM::Camera API.
 */
double fgFocus::GetExposure() const
{
 ...
 * Required by the MM::Camera API.
 */
void fgFocus::SetExposure(double exp_ms)
{
	settings_.exposure = exp_ms;
}

int fgFocus::GetAveraging() const
{
	return settings_.avarage;
}

void fgFocus::SetAveraging(int avarage)
{
	settings_.avarage = avarage;
}

// Not used
int fgFocus::SetROI(unsigned x, unsigned y, unsigned xSize, unsigned ySize)
{
	return DEVICE_OK;
}

// Not used
int fgFocus::GetROI(unsigned &x, unsigned &y, unsigned &xSize, unsigned &ySize)
{
	return DEVICE_OK;
}

// Not used
double fgFocus::GetNominalPixelSizeUm() const
{
	return 0.0;
}

// Not used
double fgFocus::GetPixelSizeUm() const
{
	return 0.0;
}
 ...
 * Required by the MM::Camera API.
 */
int fgFocus::SetBinning(int binFactor)
{
	return SetProperty(MM::g_Keyword_Binning, CDeviceUtils::ConvertToString(binFactor));
}

// Not used
int fgFocus::ClearROI()
{
	return 0;
}

int fgFocus::IsExposureSequenceable(bool &isSequenceable) const
{
	return 0;
}
...
int fgFocus::Initialize()
{
	...
	// binning
	CPropertyAction *pAct = new CPropertyAction(this, &fgFocus::OnBinning);
	int nRet = CreateIntegerProperty(MM::g_Keyword_Binning, 1, false, pAct);

	CPropertyAction *pActA = new CPropertyAction(this, &fgFocus::OnExposure);
	nRet = CreateProperty("Time [ms]", "1.0", MM::Float, false, pActA);

	CPropertyAction *pActB = new CPropertyAction(this, &fgFocus::OnAverage);
	nRet = CreateProperty("Average #", "1", MM::Integer, false, pActB);

	pAct = new CPropertyAction(this, &fgFocus::OnPixelType);
	...
	// Bit depth
	pAct = new CPropertyAction(this, &fgFocus::OnBitDepth);
...
int fgFocus::OnExposure(MM::PropertyBase *pProp, MM::ActionType eAct)
{
...
int fgFocus::OnAverage(MM::PropertyBase *pProp, MM::ActionType eAct)
{
	if (eAct == MM::BeforeGet)
	{
	}
...
int fgFocus::OnBinning(MM::PropertyBase *pProp, MM::ActionType eAct)
{
	int ret = DEVICE_OK;
...
int fgFocus::OnPixelType(MM::PropertyBase *pProp, MM::ActionType eAct)
{
	int ret = DEVICE_ERR;
	switch (eAct)
	{
...
int fgFocus::OnBitDepth(MM::PropertyBase *pProp, MM::ActionType eAct)
{
	int ret = DEVICE_ERR;
	switch (eAct)
	{
...
int fgFocus::OnPort(MM::PropertyBase *pProp, MM::ActionType pAct)
{
...
int fgFocus::GetControllerVersion(int &version)
{
...
int fgFocus::OnVersion(MM::PropertyBase *pProp, MM::ActionType pAct)
{
	if (pAct == MM::BeforeGet)
	{
